...
export function isTrustlineAlreadyExists(result: any): boolean {
  try {
    const codes = result.extras?.result_codes;
    if (codes?.operations) {
      return codes.operations.includes('change_trust_already_exists') || codes.operations.includes('op_line_full');
    }
    return false;
  } catch {
    return false;
...
export function isInsufficientBalance(result: any): boolean {
  try {
    ...
    if (codes?.operations) {
      return codes.operations.includes('op_underfunded') || codes.operations.includes('change_trust_low_reserve');
    }
    return false;
  } catch {
    return false;
