 ...
 */
export function CurrencyConverter({ amount, customSourceCurrency, className }: Props) {
  const { t } = useTranslation();
  const [usdAmount, setUsdAmount] = useState<number | null>(null);
  const { convertToUSD, isLoading, error } = useCurrencyExchange(customSourceCurrency);
  ...
  if (error) {
    return <Text className={`text-error-500 ${className}`}>{t('general.conversionError')}</Text>;
  }
  ...
  return <Text className={`text-sm text-gray-500 ${className}`}>{`≈ ${formatCurrency(usdAmount ?? 0, 'USD')}`}</Text>;
}
